...
# Data Source: Bian Yiyang (NJU) Driver behavior project
...
# >>> 6370 gender 1 (male?) vs 842 gender 2 (female).
##Tan——1==male, 2==female

### TODO: gender effect on driver score?
# >>> data gathered over 469 days, June to October.
### TODO: similar weather condition in the area over this period?
# >>> 25% of drivers less than 900 total miles driven?? what unit? km?

c("youngest" = min(Drive_clean$age, na.rm = T), "oldest" = max(Drive_clean$age, na.rm = T)) # youngest driver
...
# now with known demographic group
Drive_Anhui %>%
  select_if(~class(.x) == "numeric") %>%
  select(!starts_with("log"),
         -age,
  ...
  cor() %>%
  corrplot::corrplot(method = "number", type = "upper")
# >>> do gasoline_consumption and co2_emission measure the same thing?
##Tan——some research says they are the same thing~ 
# >>> why is time driven negatively correlated with driving score 
##Tan——(maybe longer driving time increase the chance of making mistakes?)
# >>> rapid acceleration low correlation with night driving? less traffic != faster acceleration
##Tan——i think correlation does not necessarily imply causation, so that relationship might be a coincidence 

max(Drive_clean$rapid_acceleration)
# >>> 984 rapid accelerations in 1 day?
### TODO: define outlier thresholds. IQR > 1.5?

max(Drive_clean$driving_experience, na.rm = T)
# >>> maximum driving experience of 30 months???
### TODO: find out how driving_experience was obtained

# Visualizing -------------------------------------------------------------
# Would it be useful to look at gender differences?
Drive_driving_score %>%
  ggplot(aes(x = driving_score, fill = gender)) +
  geom_histogram(bins = 25) +
  facet_wrap(~gender)
# >>> much higher driving scores when demographic is unknown
# >>> too few female drivers to make proper inferences

# rough distribution of driver ages
Drive_driving_score %>%
  filter(age > 0) %>%
  na.omit() %>%
  ggplot(aes(x = age)) +
  geom_histogram(binwidth = 1)
# >>> There are large age gaps between data points. Impact on modeling?
# >>> Data points per age group is skewed
##Tan——that must because the small sample size?!
...
### TODO: find out subgroup criteria
# >>> perhaps able to define age groups based on density?

# rough look at relationship between variables with some correlation to driver score,
# and with somewhat clear threshold
Drive_moved %>%
  ggplot(aes(x=rapid_acceleration, y=rapid_deacceleration)) +
  geom_point() +
  geom_smooth(method = "gam", formula = y ~ s(x, bs = "cs"))
# >>> seems to be general trend of more rapid accelerations is related to more rapid deaccelerations

# zoom in on the accel / deaccel graph. use vector to specify value ranges,
# single value integer for anythin smaller than that. geom_jitter used to 
# distinguish number of points at specfic value
...
# >>> seems there is a large group of observations without any infractions
Accel_Deaccl_zoom(Drive_moved, 100,100)
# >>> although seemingly there is not a very clear relationship between x and y residuals, that is
...
### TODO: eigenvalue of a drivers' "infraction count" after principal component?
...
Drive_clean %>%
  ...
  nrow()
# >>> mileage bigger than 0 but smaller than 1 seems a lot
...
# now showing how much of that is driven by month
Drive_Anhui %>%
  ...
  ggtitle("Total mileage per respondent with known demographic") +
  theme(plot.title = element_text(hjust = .5))
# >>> some drivers only drove during two or three months, others year round
##Tan——maybe the Covid-19 affects their driving behavior?
